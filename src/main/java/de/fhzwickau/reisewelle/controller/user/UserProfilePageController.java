...
public class UserProfilePageController {
    ...
    public void initialize() throws SQLException {
        ...
        // Показывать в ListView удобное отображение билета
        ticketsListView.setCellFactory(param -> new TicketListCell());

        // Обработчик клика по билету
    ...
    private static class TicketListCell extends javafx.scene.control.ListCell<Ticket> {
        @Override
        protected void updateItem(Ticket ticket, boolean empty) {
            ...
            if (empty || ticket == null) {
                ...
                String idPart = (ticket.getId() != null) ? ticket.getId().toString().substring(0, 8) : "no-id";
                setText(String.format("Ticket %s: %d Erwachsene, %d Kinder, %d Fahrräder, Preis: %.2f",
                        idPart,
                        ticket.getAdultCount(),
                        ticket.getChildCount(),
                        ticket.getBikeCount(),
